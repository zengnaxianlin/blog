...
$(function() {
  ...
  // 文章主页函数
  var postIndexFun = (function() {
    var oCol6_width = $('.col-md-12').width();  //获取主页区域布局对象
    /*
    ...
    */
    var editorFeatured = (function(){
      var $oEditorScreen = $('#editorFeatured .screen-body'),  //获取文章推荐区对象
          page = 1,                                            //初始页码
    ...
    $('#newAlbums').on('click','li',function() {
      ...
      if($(this).is('.on')) {
        ...
        var URL = '/?albumName='+encodeURIComponent('前端技术'+albumName);   // 对中文进行编码
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
          if (results.data) {
            var data = results.data.posts;              // 获取Ajax返回的音乐分类数据
            ...
            for(var k = 0; k < oThumbnail.length; k++) {
              ...
              $(oThumbnail[k]).find('a').attr('href','/post/'+ data[k]._id);
              $(oThumbnail[k]).find('h5').html(data[k].title);
              $(oThumbnail[k]).find('p').html(data[k].poster);
              var $oImg = $(oThumbnail[k]).find('img');

              if (data[k].image) {
                // 本地上传音乐图片
                $oImg.attr('src','/upload/post/'+data[k].image).attr('alt',data[k].image);
              }
            }
            // 修改新碟榜标题更多的超链接URL
            $('#newAlbums .class-top > a').attr('href','/post/results?cat='+ results.data._id +'&p=0');
    ...
    $('#kuangjs').on('click','li',function() {
      // 只有点击不同按钮才触发Ajax事件，避免对同一个按钮重复点击触发请求
      if($(this).is('.on')) {
        return;
      }else {
        var kuangName = $(this).text();                  // 获取按钮文字内容
        var URL = '/?kuangName='+encodeURIComponent('前端框架'+kuangName);   // 对中文进行编码
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
          if (results.data) {
            var data = results.data.posts;              // 获取Ajax返回的音乐分类数据
            var oThumbnail = $('#kuangjs .thumbnail'); // 获取当前音乐列表中音乐数量
            // 如果返回的数据少于当前音乐列表节点，则删除多余节点
            if(data.length < oThumbnail.length){
              var dataStart = data.length - 1;           // 设置切换到另外分类后数据起始位置
              $('#kuangjs .thumbnail:gt('+dataStart+')').remove();
              // 返回的分类电影数量大于原分类音乐数量节点则创建多出的节点
            }else if(data.length > oThumbnail.length) {
              //返回内容多于原音乐数量的创建新的节点并赋值
              for(var j = oThumbnail.length; j < data.length; j++) {
                $('#kuangjs .panel-body').append('<div class="thumbnail"><a href="" target="_blank"><img src="" alt=""/></a><div class="caption"><h5></h5><p></p></div></div>');
              }
            }
            // 切换前后音乐节点数量不变，则只替换节点内容
            oThumbnail = $('#kuangjs .thumbnail');      // 重新获取当前音乐列表中音乐节点数量
            for(var k = 0; k < oThumbnail.length; k++) {
              // 将原音乐连接、标题和海报换成切换后返回音乐数据相应内容
              $(oThumbnail[k]).find('a').attr('href','/post/'+ data[k]._id);
              $(oThumbnail[k]).find('h5').html(data[k].title);
            ...
            // 修改新碟榜标题更多的超链接URL
            $('#kuangjs .class-top > a').attr('href','/post/results?cat='+ results.data._id +'&p=0');
          // 如果切换的标题没有数据返回，则删除全部歌曲内容
          }else {
            $('#kuangjs .thumbnail').remove();
          }
        });
        // 点击当前新歌版分类添加on样式，其余删除该样式
        $(this).addClass('on').siblings('li').removeClass('on');
    ...
    $('#hotProgrammes').on('click','li',function() {
      ...
      if($(this).is('.on')){
        ...
        // 对中文进行编码
        var URL = '/?hotProName='+encodeURIComponent('后台语言'+hotProName);
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
          ...
          if(dataCars) {
            ...
            for(var i = 0; i < dataCars.length; i ++) {
              // 替换歌单标题名称和链接
              $(oThumbnail[i]).find('h5 a').html(dataCars[i].name).attr('href','/post/results?cat=' + dataCars[i]._id + '&p=0');
              // 查找每个歌单中原歌曲数目
              var oALen = $(oThumbnail[i]).find('.content a').length,
                  len = dataCars[i].posts.length;  
              // 如果返回的歌单中歌曲数量小于切换前，则将多出歌曲节点删除
              if(len < oALen) {
                var oThumbnailRemove = dataCars[i].posts.length > 0 ? $(oThumbnail[i]).find('.content a:gt('+ (dataCars[i].posts.length - 1) +')') : $(oThumbnail[i]).find('.content a') ;
              ...
              // 返回的歌曲数量大于原歌单中歌曲数量则创建新歌曲节点
                ...
                for(var z = 0; z < len - oALen; z++){
                  $('#hotProgrammes .thumbnail:eq('+ i +') .inner .content').append('<a href =/post/'+ dataCars[i].posts[z]._id +' target =_blank title = '+  dataCars[i].posts[z].title +'><p>'+ (z + 1) + '.' + dataCars[i].posts[z].title +'</p></a>');
                }
                // 当每个歌单中歌曲数量大于2首时显示...省略号
                if(dataCars[i].posts.length > 2){
                  $('#hotProgrammes .thumbnail:eq('+ i +') .inner .content').append('<span>...</span>');
                }
              }
              // 歌单中歌曲数量相同，则只替换相应内容，节点数量不变
              for(var j = 0; j < dataCars[i].posts.length; j++) {
                $(oThumbnail[i]).find('.content a:eq('+ j +')').attr('href','/post/' + dataCars[i].posts[j]._id).attr('title',dataCars[i].posts[j].title);
                $(oThumbnail[i]).find('p:eq('+ j +')').html((j + 1) + '.' + dataCars[i].posts[j].title);
              }
              var $oImg = $(oThumbnail[i]).find('img');
              // 判断每个歌单海报是否存在及是否是自行上传
              if(dataCars[i].posts[0]) {
                if(dataCars[i].posts[0].image.indexOf('http:') > -1){
                  $oImg.attr('src',dataCars[i].posts[0].image).attr('alt',dataCars[i].posts[0].image).attr('style','display: inline');
                }else{
                  // 自行上传的海报图片路径不同
                  $oImg.attr('src','/upload/post/'+dataCars[i].posts[0].image).attr('alt',dataCars[i].posts[0].image).attr('style','display: inline');
                }
              }else {
            ...
            // 修改近期热门歌单标题更多的超链接URL
            $('#hotProgrammes .class-top > a').attr('href','/post/results?pro='+ results.dataPro._id +'&p=0');
          }else {
            // 若没有数据返回则清空内容
    ...
    $('#hotArtistSongs .hot-artistTop').on('click','li',function() {
      ...
      if($(this).is('.on')){
        ...
        // 对中文进行编码
        var URL = '/?hotSongs='+encodeURIComponent('本周单曲榜'+hotSongs);
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
          if(results.data) {
            var data = results.data.posts;        // 获取Ajax返回的音乐分类数据
            ...
            for(var k = 0;k < oLi.length; k++) {
              if(data[k]) {
                //将原音乐连接、标题和海报换成切换后返回音乐数据相应内容
                $(oLi[k]).find('a').attr('href','/post/'+data[k]._id);
                $(oLi[k]).find('h5').html(data[k].title);
                $(oLi[k]).find('p').html(data[k].singer+'&nbsp;/&nbsp;'+data[k].pv+'次阅读');

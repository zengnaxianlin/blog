...
$(function() {
  ...
  var movieIndexFun = (function() {
    var oCol6_width = $('.col-md-12').width();           // 获取主页左边区域布局对象
    /*
        即将上映和正在上映点击切换事件
    */
    var gallerySwitch = (function() {
      var $oPanel = $('#scrollMoives .panel'),          // 获取顶部轮播图面板对象
          ...
          pageTotal = Math.ceil(len / 6);               // 即将电影轮播展示区总页数
      // 设置两个电影展示区总页数
      ...
      var marginWidth = $oThumbnail.outerWidth(true) - $oThumbnail.outerWidth(),
          oThumbnailWidth = (oCol6_width - marginWidth * 6) / 6;
      // 设置每张轮播海报图片的宽度
      ...
      $oTitle.on('click',function() {
        ...
        var URL = 'movieIndex/?galleryName=' + encodeURIComponent(galleryName);
    ...
    */
    $('#fliterMovies .class-top').on('click','button',function() {
      ...
      if($(this).is('.btn-primary')) {
        ...
        var URL = 'movieIndex/?fliterName=' + encodeURIComponent(fliterName + '电影');// 对中文进行编码
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
    ...
    var cinemasSearch = (function(){
      ...
      function funGetCityCinemas() {
        var cityName = $city.children('span').text(),
            ...
            URL = 'movieIndex/?cityName='+encodeURIComponent(cityName)+'&&search='+encodeURIComponent(searchName);
        // 发送Ajax请求
        funAjax(URL,'GET',function(results) {
          var data = results || [];

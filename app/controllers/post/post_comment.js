...
var PostComment = require('../../models/post/post_comment');    // 音乐数据模型
// 音乐评论后台录入控制器
exports.save = function(req,res) {
  ...
  if(_comment.cid) {
    ...
    PostComment.findById(_comment.cid,function(err,comment) {
      var reply = {
  ...
  // 简单的评论，不是对评论内容的回复
    ...
    // 将用户评论创建新对象并保存
    var comment = new PostComment(_comment);
    comment.save(function(err,comment) {
      if(err){
        console.log(err);
      }
...
exports.del = function(req,res) {
    ...
    if(did !== 'undefined') {
      // 先查找到该叠楼评论
      PostComment.findById(cid,function(err,comment) {
    ...
    // 若是点击第一条评论中的删除
      ...
      PostComment.remove({_id:cid},function(err) {
        if(err){
          console.log(err);
        }
        res.json({success:1});
